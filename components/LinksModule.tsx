            ...
            <div className="max-w-6xl mx-auto space-y-12 text-left">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-6 text-left">
                        <button
                            onClick={onBack}
                            className="p-4 bg-white dark:bg-white/5 rounded-2xl hover:bg-slate-50 dark:hover:bg-white/10 transition-all text-slate-800 dark:text-white/40 border border-slate-400 dark:border-white/5 shadow-sm active:scale-90"
                        >
                            <ChevronLeft size={24} />
                        </button>
                        <div className="text-left">
                            ...
                            <p className="text-slate-800 dark:text-white/20 text-[10px] font-black tracking-[0.3em] uppercase mt-1 text-left">Plataformas Externas de Gestão TI</p>
                        </div>
                    </div>
                ...
                <div className="flex items-center gap-3 animate-in fade-in duration-300">
                    <span className="text-[10px] font-black uppercase text-slate-700 dark:text-white/30 tracking-widest">Campus:</span>
                        ...
                        {(['Todos', 'Álvares', 'Aeroporto'] as const).map((cp) => (
                            <button
                                ...
                                className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${selectedCampus === cp
                                    ...
                                    : 'bg-white dark:bg-white/5 text-slate-800 dark:text-white/40 border-slate-400 dark:border-white/5 hover:border-slate-400 dark:hover:border-white/10 hover:text-slate-800 dark:hover:text-white shadow-sm'
                                    }`}
                            >
                                {cp}
                            </button>
                ...
                {error && (
                    <div className="bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 p-8 rounded-3xl flex flex-col items-center text-center gap-4 animate-in slide-in-from-top-4 duration-300 shadow-sm">
                        ...
                        <p className="text-slate-800 dark:text-white/40 text-[10px] leading-relaxed max-w-sm">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 bg-white dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 text-slate-700 dark:text-white border border-slate-400 dark:border-transparent px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all mt-2 shadow-sm"
                        >
                ...
                {/* Links Grid */}
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-24 text-slate-700 dark:text-white/20 gap-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredShortcuts.length === 0 ? (
                            <div className="col-span-full py-24 border-2 border-dashed border-slate-400 dark:border-white/5 rounded-[2.5rem] flex flex-col items-center justify-center text-center gap-4 opacity-70 dark:opacity-40 text-slate-700">
                                <Monitor size={48} />
                            ...
                            <div key={link.id} className="relative group text-left">
                                <a
                                    ...
                                    className="block bg-white dark:bg-slate-900/60 p-8 rounded-[2.5rem] border border-slate-400 dark:border-white/5 hover:border-indigo-500/30 dark:hover:border-indigo-500/30 transition-all hover:translate-y-[-4px] active:scale-[0.98] relative overflow-hidden h-full shadow-sm"
                                >
                                    ...
                                    <div className="text-left relative z-10">
                                        ...
                                        <p className="text-slate-800 dark:text-white/40 text-sm font-medium leading-relaxed text-left line-clamp-2 italic">
                                            {link.description}
                                        </p>
                                    </div>

                                    <div className="mt-8 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-700 dark:text-white/20 group-hover:text-indigo-600 dark:group-hover:text-white/60 transition-colors relative z-10">
