...
export const Dashboard: React.FC<Props> = ({ stats, devices, onImportClick, onExportClick, userRole }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-300 dark:border-white/5 shadow-sm gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tight">Estatísticas e Relatórios</h2>
          <p className="text-[10px] font-bold text-slate-600 dark:text-white/40 uppercase tracking-widest mt-1">Visão analítica completa do inventário</p>
        </div>
          ...
          {userRole !== 'viewer' && (
            <>
              <button 
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-3.5 bg-white dark:bg-white/5 border border-slate-300 dark:border-white/10 hover:border-indigo-500 dark:hover:border-indigo-500/50 hover:text-indigo-600 dark:hover:text-indigo-400 text-slate-700 dark:text-white rounded-2xl transition-all text-[11px] font-black uppercase tracking-widest shadow-sm active:scale-95"
              >
                <FileUp size={16} />
                Importar
              </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-300 dark:border-slate-800 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-600 dark:text-white/40 mb-6 font-medium">Equipamentos ativos em cada unidade</p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-300 dark:border-slate-800 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-600 dark:text-white/40 mb-6 font-medium">Capacidade média dos notebooks (RMM)</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-2">
            {avgBatteryHealth !== null ? (
              <>
                ...
                <p className="text-xs text-slate-600 dark:text-slate-500 text-center mt-3 font-semibold">
              ...
              <div className="text-center py-6 text-slate-500 dark:text-white/20">
                ...
                <p className="text-[10px] text-slate-600 dark:text-white/30 mt-1 max-w-[200px]">Rode o Agente Universal VNC nas máquinas dos usuários para coletar telemetria.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-300 dark:border-slate-800 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-600 dark:text-white/40 mb-6 font-medium">Departamentos com mais equipamentos em custódia</p>
          </div>
          ...
          <div className="space-y-3.5">
            {topDepartments.length > 0 ? (
              topDepartments.map((dept, index) => (
                <div key={index} className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide flex items-center gap-2">
                    <span className="w-5 h-5 rounded-md bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[10px] font-black text-slate-600">{index + 1}</span>
                    {dept.name}
                  </span>
              ...
              <div className="text-center py-8 text-slate-500 dark:text-white/20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-300 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-emerald-500/10 text-emerald-500 rounded-lg">
                <Package size={16} />
            </div>
          ...
          <p className="text-xs text-slate-600 dark:text-white/40 mb-8 font-medium">Distribuição de unidades agrupadas no catálogo</p>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-300 dark:border-slate-800 shadow-sm relative">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-indigo-500/10 text-indigo-500 rounded-lg">
                <Activity size={16} />
            </div>
          ...
          <p className="text-xs text-slate-600 dark:text-white/40 mb-8 font-medium">Classificação e divisão métrica das unidades</p>
          ...
          <div className="h-64 w-full relative" style={{ outline: 'none' }}>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-[8px] font-black uppercase tracking-widest text-slate-600 dark:text-white/40 mt-1">Total de Ativos</span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-6 mt-4">
            {pieData.map(item => (
              <div key={item.name} className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-white/40">{item.name}</span>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-300 dark:border-slate-800 shadow-sm">
        ...
        <p className="text-xs text-slate-600 dark:text-white/40 mb-6 font-medium">Últimos registros de atribuição e devolução no painel</p>

        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-slate-300 dark:border-white/5 text-slate-500 dark:text-white/30 uppercase tracking-wider font-black text-[10px]">
              ...
              {recentActivities.length > 0 ? (
                recentActivities.map((act, i) => (
                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-white/[0.01] transition-colors">
                    ...
                    <td className="py-3.5 text-slate-600 dark:text-slate-500">{act.campus || 'N/A'}</td>
                    <td className="py-3.5 text-slate-600 dark:text-slate-500">
                ...
                <tr>
                  <td colSpan={6} className="py-6 text-center text-slate-500 dark:text-white/20 font-bold uppercase tracking-wider">
...
const StatCard = ({ label, value, secondaryValue = '', subtitle, icon, borderColor, iconBgColor, iconTextColor, valueColor }: any) => (
  <div className={`p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-300 dark:border-white/5 transition-all flex flex-col justify-between shadow-sm hover:shadow-md hover:-translate-y-1 active:translate-y-0 duration-300 relative overflow-hidden ${borderColor}`}>
    <div className="flex justify-between items-start mb-4">
      <div className="flex flex-col gap-1">
        <p className="text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-white/40">{label}</p>
        <div className="flex items-baseline">
            <p className={`text-3xl font-[900] tracking-tighter leading-none ${valueColor}`}>{value}</p>
            {secondaryValue && <span className="text-sm font-bold text-slate-500 dark:text-white/30 ml-1">{secondaryValue}</span>}
        </div>
      </div>
    ...
    <div className="text-[10px] font-semibold text-slate-500 dark:text-white/30">
        {subtitle}
    </div>
  </div>
);
