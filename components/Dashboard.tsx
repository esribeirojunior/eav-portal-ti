...
export const Dashboard: React.FC<Props> = ({ stats, devices, onImportClick, onExportClick }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200/80 dark:border-white/5 shadow-sm gap-4">
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-white/40 mb-6 font-medium">Equipamentos ativos em cada unidade</p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-white/40 mb-6 font-medium">Capacidade média dos notebooks (RMM)</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-2">
            {avgBatteryHealth !== null ? (
              ...
              <div className="text-center py-6 text-slate-400 dark:text-white/20">
                ...
                <p className="text-[10px] text-slate-500 dark:text-white/30 mt-1 max-w-[200px]">Rode o Agente Universal VNC nas máquinas dos usuários para coletar telemetria.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-white/40 mb-6 font-medium">Departamentos com mais equipamentos em custódia</p>
          </div>
              ...
              topDepartments.map((dept, index) => (
                <div key={index} className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide flex items-center gap-2">
                    <span className="w-5 h-5 rounded-md bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[10px] font-black text-slate-500">{index + 1}</span>
                    {dept.name}
                  </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-200/80 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-emerald-500/10 text-emerald-500 rounded-lg">
                <Package size={16} />
            </div>
          ...
          <p className="text-xs text-slate-500 dark:text-white/40 mb-8 font-medium">Distribuição de unidades agrupadas no catálogo</p>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-200/80 dark:border-slate-800 shadow-sm relative">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-indigo-500/10 text-indigo-500 rounded-lg">
                <Activity size={16} />
            </div>
          ...
          <p className="text-xs text-slate-500 dark:text-white/40 mb-8 font-medium">Classificação e divisão métrica das unidades</p>
          ...
          <div className="h-64 w-full relative" style={{ outline: 'none' }}>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-[8px] font-black uppercase tracking-widest text-slate-500 dark:text-white/40 mt-1">Total de Ativos</span>
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-200/80 dark:border-slate-800 shadow-sm">
        ...
        <p className="text-xs text-slate-500 dark:text-white/40 mb-6 font-medium">Últimos registros de atribuição e devolução no painel</p>

        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-slate-200 dark:border-white/5 text-slate-400 dark:text-white/30 uppercase tracking-wider font-black text-[10px]">
              ...
              {recentActivities.length > 0 ? (
                recentActivities.map((act, i) => (
                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-white/[0.01] transition-colors">
                    ...
                    <td className="py-3.5 text-slate-500 dark:text-slate-400">{act.campus || 'N/A'}</td>
                ...
                <tr>
                  <td colSpan={6} className="py-6 text-center text-slate-400 dark:text-white/20 font-bold uppercase tracking-wider">
...
const StatCard = ({ label, value, secondaryValue = '', subtitle, icon, borderColor, iconBgColor, iconTextColor, valueColor }: any) => (
  <div className={`p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 transition-all flex flex-col justify-between shadow-sm hover:shadow-md hover:-translate-y-1 active:translate-y-0 duration-300 relative overflow-hidden ${borderColor}`}>
